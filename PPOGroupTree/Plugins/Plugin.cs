...
namespace PPOGroupTree.Plugins
{
    public class Plugin                            
    {
        ...
        //private ISettingPlugin[] SettingPlugins;
        ...
        public Plugin(TreeView tree, ContextMenuStrip root, ContextMenuStrip group, ContextMenuStrip student)
        {
            this.Tree = tree;
            this.rMenu = root;
            this.gMenu = group;
        ...
        private List<KeyValuePair<PluginInfor, EventHandler>> getPluginEvent
        {
            get
            {
                ...
                {
                    var tmp = new LocalEventHandler(this.Plugins[i], this.Tree);
                    var handler = tmp.GetHandler;
                    
                    result.Add(new KeyValuePair<PluginInfor, EventHandler>(this.Plugins[i].PluginInfo, handler));
                }

                //for (int i = 0; i < this.SettingPlugins.Length; i++)
                //{
                //    var tmp = new LocalEventHandler(this.SettingPlugins[i]);
                //    var handler = tmp.GetHandler;

                //    result.Add(new KeyValuePair<PluginInfor, EventHandler>(this.SettingPlugins[i].PluginInfo, handler));
                //}
